...
bot.command('fetch', async ctx => {
  ...
  if (mode === 'sheet') {
    if (!sess.sheetId) return replyBold(ctx, 'Missing sheet. Use "/sheet + &lt;URL/ID&gt;".');
...
async function startFetchFlow(ctx, email, pass, sheetId, dateStr = null) {
  ...
  const prev = sessionMap.get(ctx.chat.id) || {};
  sessionMap.set(ctx.chat.id, { 
    ...prev,
    rows,
    dateStr: ds,
    sheetId,
...
app.get('/oauth2callback', async (req, res) => {
  ...
  googleCreds.set(chatId, { ...prev, refresh_token: tokens.refresh_token });  
    await bot.api.sendMessage(
      chatId,
       '<b>Authorized! fetching…</b>',
      { parse_mode:'HTML' }
    );
   // After a 3‑second delay, only if the refresh_token is really set...
   setTimeout(async () => {
     const creds = googleCreds.get(chatId);
     const sheetId = sheetMap.get(chatId);
     if (!creds?.refresh_token || !sheetId) {
       // Something went wrong or they never sent /sheet – abort.
       return;
     }

     // Build a minimal context for startFetchFlow
     const fakeCtx = {
       chat: { id: chatId },
       reply: (text, extra) => bot.api.sendMessage(chatId, text, extra),
     };

     // Pull their Flyfone credentials
     const { email, pass } = flyfoneCreds.get(chatId);

     // And invoke your normal fetch flow
     try {
       await startFetchFlow(fakeCtx, email, pass, sheetId);
     } catch (err) {
