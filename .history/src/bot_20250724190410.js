...
async function beginSheetFlow(ctx) {
  ...
  return ctx.reply('<b>Select a date to export:</b>', { reply_markup: calendar });
}
...
bot.callbackQuery(/^chat_agent:(.+)\|(.+)$/, async ctx => {
  ...
  const filtered = rows
    .slice(1)
    .filter(r => r[6] === team && r[5] === agent);
  const total = filtered.length;
  const counts = filtered.reduce((acc, r) => {
    const st = r[7]?.toString().trim();
    acc[st] = (acc[st] || 0) + 1;
    return acc;
  }, {});
  await ctx.editMessageText(
...
bot.callbackQuery(/^team:(.+)$/, async ctx => {
  ...
  // filter by lowercase match
  const filtered = rows.slice(1)
    .filter(r => r[6]?.toString().toLowerCase() === teamKey);

  const output = [[
    'Caller','Team','Callee','Status','Duration (s)',
    'Talktime (s)','Hangup By','Call Date','Call Time','End Time'
  ]];
  filtered.forEach(r => {
    output.push([
      r[5], r[6], r[7], r[8],
      r[9], r[10], r[11], r[1], r[2], r[3]
    ]);
  });
...
await ctx.editMessageText(
  `<b>Appended ${filtered.length} rows for team “${teamKey}” on ${dateStr}</b>`,
  { parse_mode: 'HTML' }
);
  } catch (err) {
    console.error(err);
...
// ─────────────────────────────────────────────────────────────────────────────
// /logout → clear saved creds & cookies
// ─────────────────────────────────────────────────────────────────────────────
bot.command('logout', async ctx => {
  ...
  // wipe the live axios cookie jar
  await jar.removeAllCookies();    // or await jar.removeAllCookies()

  return ctx.reply(
    '<b>You have been logged out.</b>\n' +
    'Your Flyfone credentials and session cookies have been cleared.\n' +
    'Use /fetch to log in again.',
    { parse_mode: 'HTML' }
  );
});
