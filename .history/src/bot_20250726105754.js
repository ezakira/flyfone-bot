...
bot.callbackQuery(/^linkSheet:yes$/, async ctx => {
  ...
  sessionMap.set(chatId, { connectSheet: true, mode: 'sheet' });
  await ctx.answerCallbackQuery();

  // Send OAuth consent link first
  ...
  await ctx.reply('<b>Please authorize access to your Google Sheets:</b>', {
    reply_markup: {
...
async function beginSheetFlow(ctx) {
  ...
  if (!creds) {
    ...
    console.log('💡 [DEBUG] beginSheetFlow set step to sheet_await_email');
...
bot.filter(
  ...
  async ctx => {
    ...
    console.log('💡 [DEBUG-cal] creds for', chatId, userCreds.get(chatId));
...
bot.on('message:text', async ctx => {
  const chatId = ctx.chat.id;
  const sess   = sessionMap.get(chatId);
  console.log('💡 [DEBUG] message:text for chatId=', chatId, 'sess=', sess, 'userCreds=', userCreds.get(chatId));
  if (!sess) return;

   // ←—— ADD THIS:
  if (['sheet_await_email','chat_await_email'].includes(sess.step)) {
    // store email and advance to the password prompt
    sess.email = ctx.message.text.trim();
    sess.step  = sess.step === 'sheet_await_email'
                ? 'sheet_await_pass'
                : 'chat_await_pass';
    sessionMap.set(chatId, sess);
    console.log('💡 [DEBUG] advanced to step=', sess.step);
...
if (['sheet_await_pass','chat_await_pass'].includes(sess.step)) {
  ...
  try {
    ...
    userCreds.set(chatId, { email: sess.email, pass });
     console.log('💡 [DEBUG] credentials stored:', userCreds.get(chatId));

    if (sess.step === 'sheet_await_pass') {
      // Sheet‑mode: still want to go straight to calendar
      sess.step = 'sheet_await_date';
      sessionMap.set(chatId, sess);
       console.log('💡 [DEBUG] moving to sheet_await_date');
      ctx.session.calendarOptions = { defaultDate: dayjs().subtract(1,'day').toDate() };
      return ctx.reply(
    ...
    console.error('⚠️ [DEBUG] login failed:', err);
...
app.get('/oauth2callback', async (req, res) => {
  ...
  try {
    ...
    userCreds.set(chatId, { refresh_token: tokens.refresh_token });
