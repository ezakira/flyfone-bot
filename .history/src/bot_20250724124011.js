...
bot.callbackQuery(/^team:(.+)$/, async ctx => {
  ...
  try {
    ...
    await ctx.editMessageText(`**Wrote ${filtered.length} rows for team "${teamKey}" on ${dateStr}**`, { parse_mode: 'HTML' });
  } catch (err) {
    await ctx.editMessageText(`**Write failed: ${err.message}**`, { parse_mode: 'HTML' });
  } finally {
    sessionMap.delete(chatId);
  }
});
...
async function startFetchFlow(ctx, email, pass, sheetId, dateStr = null) {
  ...
  try {
    ...
    await ctx.replyBold('Select a team:', {
      reply_markup: kb,
      parse_mode: 'HTML',
    });
