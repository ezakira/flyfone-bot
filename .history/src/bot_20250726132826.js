...
if (creds?.refresh_token) {
  return ctx.reply(
    '<b>You are already authorized.</b>\n\nSend /sheet+&lt;URL&gt; to link your sheet.',
    { parse_mode: 'HTML' }
  );
}
  ...
  return ctx.reply('<b>Please authorize access to your Google Sheets:</b>', {
    reply_markup: {
...
bot.command('fetch', async ctx => {
  const chatId = ctx.chat.id;
  const sess   = sessionMap.get(chatId);
const mode = sessionMap.get(chatId)?.mode;
const creds = flyfoneCreds.get(chatId);
if (!creds) return replyBold(ctx, 'You’re not logged in. Please /start.');
if (!mode) return replyBold(ctx, 'No mode selected. Use /mode to pick one.');

if (mode === 'sheet') return beginSheetFlow(ctx);
