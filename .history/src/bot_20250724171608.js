...
async function beginFetch(ctx) {
  const chatId = ctx.chat.id;
  // pull both flags from sessionMap and sheetMap
  const sess    = sessionMap.get(chatId) || {};
  const sheetId = sheetMap.get(chatId);

  // if they opted to link a sheet but haven’t yet
  if (sess.connectSheet && !sheetId) {
    return replyBold(ctx, 'No sheet linked. Use /setSheet URL or ID.');
  }

  // save sheetId & creds into session for downstream
sess.step = 'await_email';
sessionMap.set(chatId, sess);

  // if we already have creds, skip to date picker
  const creds = userCreds.get(chatId);
  if (creds) {
    sessionMap.set(chatId, { ...sessionMap.get(chatId), step:'await_date', email:creds.email, pass:creds.pass });
...
// ─────────────────────────────────────────────────────────────────────────────
// Handle the grammy‑calendar date pick
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// Handle calendar picks
// ─────────────────────────────────────────────────────────────────────────────
bot.filter(
  ...
  async ctx => {
    ...
    const dateStr = dayjs(picked).format('YYYY-MM-DD');         // normalized
...
bot.on('message:text', async ctx => {
  ...
  const date = ctx.message.text.trim();
  // helper to normalize whatever is in r[1]
  function normDate(cell) {
    if (cell instanceof Date) return dayjs(cell).format('YYYY-MM-DD');
    return dayjs(cell).isValid()
      ? dayjs(cell).format('YYYY-MM-DD')
      : cell.toString();
  }

  // filter by team, agent, AND normalized date
  const rows = sess.rows.slice(1).filter(r => {
    const teamMatch  = r[6].toString().toLowerCase() === sess.team;
    const agentMatch = r[5] === sess.agent;
    const dateMatch  = normDate(r[1]) === date;
    return teamMatch && agentMatch && dateMatch;
  });
...
async function startFetchFlow(ctx, email, pass, sheetId, dateStr = null) {
  ...
  try {
    ...
    console.log('▶ downloaded rows:', rows.length, 'header:', rows[0]);

const raw = rows.slice(1).map(r => r[6]?.toString()||'').filter(t=>t);
...
const teams = Array.from(new Set(raw)).map(label => ({
  ...
  label
    ...
    const kb = new InlineKeyboard();
teams.forEach(t => {
  kb.text(t.label, `team:${t.key}`).row();
});
