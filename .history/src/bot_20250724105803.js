...
bot.command('start', ctx => replyBold(ctx, 'Up and running via polling.'));
...
bot.on('message:text', async ctx => {
  ...
  if (sess.step === 'await_email') {
    ...
    return replyBold(ctx, 'Got it. Now enter your Flyfone password:');
  }
...
async function startFetchFlow(ctx, email, pass, sheetId) {
  await replyBold(ctx, 'Downloading Flyfone report…');
  ...
  try {
    ...
    await ctx.reply('**Select a team:**', { reply_markup: kb, parse_mode: 'Markdown' });
