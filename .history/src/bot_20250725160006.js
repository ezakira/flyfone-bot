...
bot.command('summary', async ctx => {
  ...
  if (parts.length < 2) {
    return replyBold(ctx, 'Usage: /summary <date|today|yesterday> <TeamName>');
  }
  ...
  let rows;
  try {
    // Download the full-day report
    rows = await downloadFlyfoneReport(dateStr, creds.email, creds.pass);
  } catch (err) {
    return replyBold(ctx, `Error fetching report: ${err.message}`);
  }

  // 3) Filter for team & group by agent
  //   in your sheet layout: team is column index 6, agent is index 5
  const dataRows = rows.slice(1).filter(r => (r[6] || '').toString() === team);
  ...
  const lines  = Object.entries(counts)
    .sort((a, b) => b[1] - a[1])       // optional: highest first
