...
bot.callbackQuery(/^linkSheet:(yes|no)$/, async ctx => {
  ...
  if (choice === 'yes') {
    return replyBold(ctx, 'Great—please /setSheet <ID or URL> now.');
  } else {
...
bot.callbackQuery(/^team:(.+)$/, async ctx => {
  ...
  try {
    ...
    await ctx.editMessageText(`<b>Wrote ${filtered.length} rows for team "${teamKey}" on ${dateStr}</b>`, { parse_mode: 'HTML' });
  } catch (err) {
    await ctx.editMessageText(`<b>Write failed: ${err.message}</b>`, { parse_mode: 'HTML' });
  } finally {
    sessionMap.delete(chatId);
  }
});
