...
import { flyfoneCreds, googleCreds } from './state.js';
...
bot.on('message:text', async ctx => {
  const chatId = ctx.chat.id;
  const sess   = sessionMap.get(chatId);
  console.log('💡 [DEBUG] message:text for chatId=', chatId, 'sess=', sess, 'flyfoneCreds=', flyfoneCreds.get(chatId));
  if (!sess) return;
...
if (['sheet_await_pass','chat_await_pass'].includes(sess.step)) {
  ...
  try {
    ...
    flyfoneCreds.set(chatId, { email: sess.email, pass });
        console.log('💡 [DEBUG] credentials stored:', flyfoneCreds.get(chatId));
