...
bot.filter(
  ...
  async ctx => {
    ...
    console.error('❌ Calendar error:', err.stack || err);
    return replyBold(ctx, `Error: ${err.message}`);
  }
});
...
bot.on('message:text', async ctx => {
  ...
  function normDate(cell) {
    ...
    const n = Number(cell);
    if (!isNaN(n)) {
      return dayjs(new Date((n - 25569) * 86400000)).format('YYYY-MM-DD');
    }
  ...
  console.error('❌ stats handler error:', err.stack || err);
    return replyBold(ctx, `Internal error: ${err.message}`);
  }
});
