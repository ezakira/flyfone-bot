  ...
  if (sess.step === 'await_pass') {
    ...
    try {
      ...
      if (err.message === 'Incorrect email or password') {
        ...
        sessionMap.set(chatId, sess);
        return replyBold(ctx, 'Wrong credentials—please enter your Flyfone email again:');
      }
...
bot.callbackQuery(/^team:(.+)$/, async ctx => {
  ...
  // filter down to this team
  const rows = sess.rows.slice(1).filter(r => 
    r[6]?.toString().toLowerCase() === teamKey
  );

  // if user opted in for sheet, write immediately
  ...
  // otherwise, branch into “choose agent”
  sess.team = teamKey;
  ...
  const kb = new InlineKeyboard();
  agents.forEach(a => kb.text(a, `agent:${a}`).row());
...
// 3) Text handler to catch the date
bot.on('message:text', async ctx => {
  const chatId = ctx.chat.id;
  const sess = sessionMap.get(chatId);
  // only trigger if we’re waiting for a date
  if (!sess?.agent || !sess?.team) return;

  const date = ctx.message.text.trim();
  const filtered = sess.rows.slice(1).filter(r =>
  ...
  await ctx.replyWithHTML(
    ...
    `<b>Date:</b> ${date}\n` +
    `<b>Calls:</b> ${total}\n` +
    `<b>Answered:</b> ${stats.Answered||0}\n` +
    `<b>Cancelled:</b> ${stats.Cancelled||0}\n` +
    `<b>Busy:</b> ${stats.Busy||0}`
  );
  sessionMap.delete(chatId);
});
